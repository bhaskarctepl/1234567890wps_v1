...
public class WearablesUtils {
	...
	public static int getRequestFrom(String platform) {
		int reqesutFrom = 0;
		if (platform == null || platform.trim().isEmpty()) {
			return NumberUtils.INTEGER_ZERO;
		} else if (Constants.REQUEST_FROM_WEARABLES.equalsIgnoreCase(platform.trim())) {
			reqesutFrom = 1;
		} else if (Constants.REQUEST_FROM_ANALYTICAL.equalsIgnoreCase(platform.trim())) {
			reqesutFrom = 2;
		}
	...
	public static String covertToReadableTime(long totalSeconds) {
		String timeText = "";
		if (totalSeconds > 0) {
